...
def verify_first_gate(filename, pdf_0_0, manual_date=None):
    if manual_date:
        ...
        if match_y and match_m:
            ...
            return False, "理由: ファイル名に年月（202X, X月）が見つかりません。", None
    ...
    if last_day_calc == last_day_pdf and first_w_calc == first_w_pdf:
        ...
        reason = f"【不一致】 期待値: 1日({first_w_calc}), 末日({last_day_calc}日) / PDF値: 1日({first_w_pdf}), 末日({last_day_pdf}日)"
        return False, reason, None

def analyze_pdf_structural(pdf_stream, master_keys, filename):
    with open("temp.pdf", "wb") as f: f.write(pdf_stream.getbuffer())
    try:
        tables = camelot.read_pdf("temp.pdf", pages='1', flavor='lattice')
        if not tables: return None, "理由: PDFから表構造を抽出できませんでした。"
        raw_df = tables[0].df
        raw_0_0 = str(raw_df.iloc[0, 0])
        
        success, msg, date_info = verify_first_gate(filename, raw_0_0)
        ...
        for i in range(2, len(raw_df), 2):
            ...
            if name and name.lower() != 'nan' and normalize_text(name) != loc_norm:
