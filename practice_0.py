...
def extract_target_data(df, target_staff, location):
    """第3関門：my_daily_shiftとother_daily_shiftの抽出"""
...
def parse_honmachi_time(detail):
    """メイン工程6：本町時間抽出（例：9①14 -> 9:00, 14:00）"""
    match = re.search(r'(\d{1,2})[①-⑩](\d{1,2})', str(detail))
    if match:
        return f"{int(match.group(1)):02d}:00", f"{int(match.group(2)):02d}:00"
    return "", ""

def shift_cal(key, target_date, col_idx, shift_info, detail_info, other_staff_shift, time_schedule, final_rows):
    """メイン工程の実装"""
