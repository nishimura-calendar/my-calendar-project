...
def normalize_text(text):
    """全角・半角、空白を統一し、紐付けの正確性を担保する"""
    if not isinstance(text, str): return ""
    return re.sub(r'[\s　]', '', unicodedata.normalize('NFKC', text)).lower()

def get_master_data_from_df(df):
    """スプレッドシートの行列(iloc)から勤務地と時程定義を抽出"""
    # A列(0列目)から勤務地名を特定
...
def rebuild_shift_table(pdf_path, target_name, expected_days, expected_weekday, master_locations):
    ...
    if not detected_loc:
        return None, "検問失敗：スプレッドシートの勤務地名がPDF内に見つかりません。"

    # --- ② 日数・第一曜日の検問（ファイル違いを遮断） ---
    ...
    for i in range(len(combined_df)):
        # 行(row)全体を文字列化し、中線の影響を受けずに記号を順番に抽出
