...
def generate_calendar_records(year, month, location, time_schedule_df, my_daily_shift_df, other_staff_shift_df):
    ...
    for col_idx in my_daily_shift_df.columns:
        ...
        info = str(my_daily_shift_df.iloc[0, col_idx-1]).strip()       # 上段（勤務記号）
        sub_info = str(my_daily_shift_df.iloc[1, col_idx-1]).strip()   # 下段（資格・休憩等）
        
        # ”なし”の時 →to_なし=""、"なし"の時 from_なし→="" と表記するように変更。
        if info == "なし": info = ""
