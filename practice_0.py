...
def time_schedule_from_drive(service, file_id):
    """
    基本事項に則り、スプレッドシートを .xlsx としてダウンロードして読み込む。
    A列=勤務地, B列=巡回区域, C列=ロッカ, D列以降=時間行
    """
    # スプレッドシートIDの場合は export、通常のファイルIDの場合は get_media
    try:
    ...
    for sheet_name, df in all_sheets.items():
        ...
        # Pandasのバージョン互換性のため map を使用 (旧 applymap)
        if hasattr(df, 'map'):
            df = df.map(clean_val)
        else:
...
def extract_workplace_from_header(header_text):
    ...
    num_newlines = str(header_text).count('\n')
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                if not found_indices:
                    continue # スタッフがいなければこのテーブルはスキップ
...
def shift_cal(key, target_date, col, shift_info, my_daily_shift, other_staff_shift, time_schedule, final_rows):
    """
    打合.pyのロジック反映
    基本事項：B列(index 1)=巡回区域(シフトコード), D列(index 3)以降=時間行
    """
    # 終日イベント(勤務地_シフト名)
    final_rows.append([f"{key}_{shift_info}", target_date, "", target_date, "", "True", "", key])
    
    sched_clean = time_schedule.copy()
    # B列(index 1)からシフトコードを探す
