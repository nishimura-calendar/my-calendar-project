...
def get_calc_date_info(y, m):
    ...
    # 0:月, 1:火, 2:水, 3:木, 4:金, 5:土, 6:日 を返す
    w_idx = calendar.weekday(y, m, 1)
    w_list = ["月", "火", "水", "木", "金", "土", "日"]
    first_w = w_list[w_idx] # 正しい曜日を取得
...
def analyze_pdf_structure(pdf_path, y, m):
    ...
    # PDF内の[0,1]以降から最大の日付を探す（または[0,0]から抽出）
    days_in_pdf = [int(n) for n in re.findall(r'\d+', raw_0_0)]
    pdf_last_day = max(days_in_pdf) if days_in_pdf else 0
    
    # PDF内の[1,1]や[0,0]から「最初の曜日」を探す
    pdf_first_w_match = re.search(r'[月火水木金土日]', str(df.iloc[1, 1]))
    if not pdf_first_w_match: # [1,1]にない場合は[0,0]などから
