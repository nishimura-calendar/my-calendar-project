    ...
    if not my_time_shift.empty:
        ...
        for t_col in range(2, time_schedule.shape[1]):
            ...
            if current_val == prev_val and last_row_idx != -1:
                ...
                final_rows.append([f"【{current_val}】", target_date, start_t, target_date, end_t, "False", "", key])
    ...
    if 'pdf_files' in st.session_state and st.session_state['pdf_files']:
        ...
        if st.button("② 解析を実行"):
            with st.spinner("解析中..."):
                ...
                # エラー回避のため、shift_dicが作成された直後の同じブロック内でループを回す
                for key, data_list in shift_dic.items():
                    if len(data_list) < 3:
                        continue
                    
                    rows_res = []
                    my_s, t_s = data_list[0], data_list[2] # 自分のシフト表と時程表
