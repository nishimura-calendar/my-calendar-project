    ...
    if y and m:
        ...
        if res:
            ...
            if location in st.session_state.time_dic:
                st.success(f"勤務地「{location}」照合成功")
                    ...
                    if shift_data:
                        ...
                        if st.button("カレンダーCSV生成"):
                            ...
                            for col_idx in range(1, my_shift.shape[1]):
                                ...
                                s_info = str(my_shift.iloc[0, col_idx]).strip()
                                d_info = str(my_shift.iloc[1, col_idx]).strip()
                                if s_info and s_info != "なし":
                                    p0.shift_cal(target_staff, target_date, col_idx, s_info, d_info, other_shift, t_schedule, final_rows)
                            ...
                            st.download_button("CSV保存", csv, f"{target_staff}.csv", "text/csv")
            else:
                st.error(f"「{location}」は時程表に登録されていません。")
