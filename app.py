...
def display_pdf_as_image(pdf_path):
    """PDFの1ページ目を画像に変換して画面に美しくプレビュー表示"""
    try:
        doc = fitz.open(pdf_path)
        page = doc.load_page(0)
        pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
        img_bytes = pix.tobytes("png")
        st.image(img_bytes, caption="アップロードされたPDFの確認プレビュー", use_container_width=True)
        doc.close()
    except Exception as e:
        st.warning(f"PDFプレビュー画像の生成に失敗しました: {e}")

def stop_with_pdf_image_only(error_text, pdf_path):
    """エラーメッセージとPDF画像のみを表示して処理を安全に停止（関門失敗時用）"""
    st.error(error_text)
...
if uploaded_file is not None:
    ...
    if match_year_month:
        ...
        st.info(f"📂 ファイル名から自動判定: **{y}年{m}月度** のデータを処理します。")
    else:
        # ファイル名から抽出できない場合は手動入力を促す
    ...
    # 抽出された純粋な勤務地コード(C)を取得
    location_c = parse_result["location"]
    master_keys = list(st.session_state.time_master.keys())

    # 【第2関門チェック】時程表マスターに完全一致するキーが存在するか
    if location_c not in master_keys:
        error_msg = f"【第2関門失敗】 勤務地「{location_c}」が時程表に設定されていません。確認が必要です。(利用可能キー: {master_keys})"
        stop_with_pdf_image_only(error_msg, tmp_path)

    # 両関門を突破した場合
    st.success(f"🎉 第1・第2関門を無事通過！ 勤務地キー: **{location_c}**")
    ...
    if target_staff:
        ...
        if extracted:
            ...
            if st.button("Googleカレンダー登録用CSVを生成する"):
            ...
            # CSVが生成されていればプレビューとダウンロードボタンを表示
            ...
            st.error(f"エラー: 選択されたスタッフ「{target_staff}」のシフトデータをパース内から分離できませんでした。")
